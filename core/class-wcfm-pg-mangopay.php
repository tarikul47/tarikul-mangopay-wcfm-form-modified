...
class WCFM_PG_MangoPay
{
	...
	public function wcfmmp_custom_pg_withdrawal_charges($withdrawal_charges, $wcfm_withdrawal_options, $withdrawal_charge)
	{
		...
		$payment_withdrawal_charges = array(
			"withdrawal_charge_" . $gateway_slug => array(
				'label' => $gateway_label . __('Charge', 'wcfm-pg-mangopay'),
				'type' => 'multiinput',
				...
				'value' => $withdrawal_charge_brain_tree,
		...
		try {
			...
			if ($kyc_documents) {
				...
				foreach ($kyc_documents as $kyc_document) {
					...
					if ($kyc_document->Status == 'VALIDATED' && !in_array($kyc_options[$kyc_document->Type], $validated))
						$validated[] = $kyc_options[$kyc_document->Type];
				}
			}
		} catch (Exception $e) {
	...
	public function mangopay_kyc_list($mp_user_id, $user_mp_status, $user_business_type)
	{
		...
		try {
			...
			if ($kyc_documents) {
				...
				foreach ($kyc_documents as $kyc_document) {
					...
					if (!in_array($kyc_options[$kyc_document->Type], $submitted))
						$submitted[] = $kyc_options[$kyc_document->Type];
				}
			}
		} catch (Exception $e) {
		...
		if (count($submitted) > 0)
			$kyc_options = array_diff($kyc_options, $submitted);

		if (count($remove_kyc_type) > 0)
			$kyc_options = array_diff($kyc_options, $remove_kyc_type);
	...
	public function wcfmmp_custom_pg_vendor_setting($vendor_billing_fields, $vendor_id)
	{
		...
		include_once (plugin_dir_path(__FILE__) . '../template/generate-fields.php');
		//echo "MP";
		//	return array_merge($vendor_billing_fields, $user_update_field);
		return array_merge($vendor_billing_fields, $generate_fields);
		//	} else {
		/**
