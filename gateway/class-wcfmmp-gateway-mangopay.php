...
class WCFMmp_Gateway_MangoPay extends WCFMmp_Abstract_Gateway
{
	...
	public function process_payment($withdrawal_id, $vendor_id, $withdraw_amount, $withdraw_charges, $transaction_mode = 'auto')
	{
		...
		if ($this->validate_request()) {
			...
			$status = $this->verify_mangopay_payout($mp_payout->Id);
	...
	// 				[
	// 					'class-wcfmmp-gateway-mangopay.php',
	// 					'verify_mangopay_payout - function',
	// 					$PayOut,
	// 				],
	// 				true
	// 			)
	...
	private function verify_mangopay_payout($payout_id)
	{
		...
		do {
			...
			switch ($PayOut->Status) {
				...
				case \MangoPay\PayOutStatus::Created:
					// Keep looping until the status is not "CREATED"
					continue 2; // Continue the outer loop iteration
					break;
		...
		} while ($PayOut->Status === \MangoPay\PayOutStatus::Created);
